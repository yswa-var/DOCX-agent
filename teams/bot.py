...
class DOCXAgentBot(ActivityHandler):
    ...
    async def _handle_approval(self, turn_context: TurnContext, user_id: str, message: str, user_profile: Dict) -> Dict[str, Any]:
        ...
        is_approval = normalized in ["yes", "/approve", "approve"]
        ...
        if not session_id:
            logger.error("Unable to process approval: no pending session id found")
            raise ValueError("Missing pending approval session. Please request the operation again.")

        response = await self._send_approval_decision(
            user_id=user_id,
    ...
    async def _send_to_backend(self, user_id: str, message: str, user_profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("requires_approval"):
                ...
                logger.info(f"Approval required for session: {result.get('session_id')}")
    ...
    async def on_submit_action(self, turn_context: TurnContext):
        ...
        try:
            ...
            if session_id:
                user_profile["pending_session_id"] = session_id
                await self._persist_pending_session(turn_context, session_id)
